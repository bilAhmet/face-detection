...
def detect_faces(cascade, test_image, scaleFactor = 1.1):
    ...
    # saving every face cropped from the main frame in the same directory
    for (x, y, w, h) in faces_rect:
        sub=image_copy[y:y+h, x:x+w]
        cv2.imwrite(str(time.time())+'.png',sub)
...
#loading image
test_image2 = cv2.imread('old.png')
